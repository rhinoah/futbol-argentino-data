...
|---|---|---|---|---|
| 43 804 | 107 | 149 | 822 | 298 |

## 2004–2026 — Cerrado

**Todo lo que Wikipedia da de esta era está adentro**

`41 311 ` partidos. Lo único que queda son verificaciones de calidad, no partidos faltantes: del corpus entero quedan `292` avisos abiertos y cero graves, y todos los que quedan pueden mover un dato.
...
- **Las 20 discrepancias entre el cuadro y la grilla.** Empezaron en **56** y las cinco *fases finales que sólo existían como cuadro* ya están todas adentro: `Argentino A 2012-13` por el lector de tablas de llaves, y desde fuentes externas el `Argentino A 2005-06` (58 partidos), las semis y la final del `Argentino A 2011-12` (6), el Reducido de la `Primera C 2008-09` (10) y el de la `Primera C 2011-12` (14, desde ESPN, la única que RSSSF no cubre). El problema era siempre el mismo — un cuadro dibuja quién jugó contra quién pero no dice quién fue local, y la convención *arriba es local en la ida* acierta apenas el **55.6%** contra 761 patas donde la grilla sí lo dice.
  - **Lo que queda ya no es «falta la localía» sino otra cosa.** `Argentino A 2004-05` (16) es el bloque grande y es distinto: ahí la grilla **no tiene ningún partido** entre esos clubes, así que no es que falte la localía sino la fase entera. RSSSF la publica en formato **compacto** —`Aldosivi 0-1 1-3 Luján de Cuyo` con la fecha como *rango*—, y el lector sólo entiende el expandido a propósito: un rango no es una fecha. Hay un blog (`josecarluccio`) con las fechas exactas de la 2ª fase del Apertura 2004, corroborado por RSSSF en marcadores y localía, pero **sólo cubre 14 partidos** y tiene una errata demostrable en la misma página (`01/02/2004` por `01/12/2004`), así que hay que pasarlo por un chequeo de coherencia antes de cargarlo.
  - **`Argentino A 2012-13` (2) y `Argentino A 2005-06` (2)** son desacuerdos a arbitrar, no partidos que falten: el cuadro y la grilla se dan marcadores distintos para el mismo cruce. Piden un tercer testigo.
  - **El mecanismo está hecho y probado**, y sirve para las dos fuentes: `rsssf.leer_llaves` / `espn.leer_llaves` para leer, y `build.sin_repetir` para no duplicar lo que la página ya trae —cruzando por par de clubes **más** fecha, comparando **canonizado**, y avisando tanto cuando discrepan en la fecha como cuando **la localía está al revés** con la misma fecha—.
## 1997–2003 — Cerrado
