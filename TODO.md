...
- **Las dos temporadas de RSSSF que faltan.** `2006-07` (334 partidos) y `2008-09` (400) ya entraron, las dos con cero graves y `source` acreditando a las dos fuentes. Faltan `2007-08` (`arg3-int08`) y encontrar la de `2009-10`, que **no** está en `arg3-int10` — da 404 —. `2007-08` no tiene la forma de las otras: sus secciones son `First Phase - Fase Campeonato`, `Group A` —*Group*, no *Zona*— y rondas `Interzonal Group A-B` intercaladas, así que hay que mirarla antes de forzar el mapa. Lo bueno: la regla del interzonal ya está escrita y la temporada **lee 400 partidos** limpios (144/112/144). Lo que frena el mapa es que RSSSF **le pega la cancha al nombre del club** — `Gimnasia y Esgr. (CdU) (at Huracán-C)`, `(at Libertad)` —, así que el Group B declara 18 nombres donde hay 16. La sede **ya se despega** y `Talleres` resultó ser el `(P)` olvidado en una sola línea. La estructura ya está medida y es **la misma que 2008-09**: `Group A` y `Group B` de RSSSF son el *mismo* plantel de 16 clubes —dos etapas, no dos zonas— y esos 16 son `Zona 1` + `Zona 2` de la página; `Group C` son los 9 de `Zona 3`. Así que **la zona de un partido no puede salir del rótulo de RSSSF**: sale del plantel de sus dos clubes. Eso cierra: de los 431 partidos, los únicos que cruzan zona son **40, todos `Zona 1` × `Zona 2`**, que es exactamente el interzonal. Lo que falta no es el mapa sino **7 huecos del padrón**, y aparecen de los dos lados a la vez: `Racing (C)` ↔ `Club Atlético Racing (Córdoba)`, `Rivadavia (L)` ↔ `Club Rivadavia de Lincoln`, `Dep. Santamarina` ↔ `Club Deportivo Santamarina` —esos tres el padrón no los conoce por ningún lado— más `Alumni`, `Juv.Antoniana`, `Gimnasia y Esgr. (CdU)` y `Talleres`, que la página sí resuelve y son entrada de mapa.
