...
|---|---|---|---|---|
| 43 969 | 56 | 149 | 916 | 362 |

## 2004–2026 — Cerrado

**Todo lo que Wikipedia da de esta era está adentro**

`41 502` partidos. Lo único que queda son verificaciones de calidad, no partidos faltantes: del corpus entero quedan `200` avisos abiertos y cero graves.
...
- **Quedan 56 partidos sin fecha, y 37 son de una sola temporada.** Eran `148`. Casi todo lo que se cerró estaba en fuentes que el repo ya bajaba y no leía: la fase regular de los archivos de RSSSF **no lleva encabezado de zona** cuando el torneo no tiene zonas, y sin esa clave el lector devolvía cero partidos sin un aviso; desde 2010-11 las divisiones **comparten archivo** y hay que acotar la sección —por arriba *y por abajo*— o entran partidos de otro torneo; un título como `Third Phase Reválida` no era título de nada; y «repetido» incluía al partido que la página publica **sin día**, así que el cruce lo reconocía y se iba igual a `sin-fecha/`. Lo que queda: 37 del `Argentino A 2004-05` —el resto de su formato compacto, cuya fecha es un *rango*—, 2 del `2010-11` que son un empate con la localía espejada entre las dos fuentes, y 17 sueltos.
