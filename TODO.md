...
- **Dos marcadores de la Copa Argentina 2018-19 que la página se desmiente sola.** La grilla dice `Newell's 0-0 Villa Mitre, pen 5-3` y `Almagro 1-1 Atlético de Rafaela, pen 3-4`, o sea que pasaron Newell's y Rafaela. Pero **Villa Mitre juega los octavos y Almagro le gana a Boca en la ronda siguiente y llega a cuartos**, en esa misma página. Y el cuadro de la página dice `Newell's 1 - 2 Villa Mitre`. Los encuentra sólo la cadena de llaves: el chequeo del cuadro se queda callado. Quedan abiertos a propósito — cuál de las dos afirmaciones vale no se decide desde adentro de una fuente que hace las dos, y eso pide un testigo de afuera.
  - **patas y no rondas** — `Partidos de ida` y `Partidos de vuelta`, 135 partidos cada una, la familia más grande. La llave es `Reválida`, que es una FASE, así que la página rotula la pata y nunca dice si es 16avos, octavos o cuartos: **el dato no está en ningún campo**.
  - **cosas que no son rondas** — `Promoción 1`, `Desempate`, `Tabla de descenso`, `Partidos` (26).
