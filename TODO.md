...
- **Las dos temporadas de RSSSF que faltan.** `2006-07` (334 partidos) y `2008-09` (400) ya entraron, las dos con cero graves y `source` acreditando a las dos fuentes. Faltan `2007-08` (`arg3-int08`) y encontrar la de `2009-10`, que **no** está en `arg3-int10` — da 404 —. `2007-08` no tiene la forma de las otras: sus secciones son `First Phase - Fase Campeonato`, `Group A` —*Group*, no *Zona*— y rondas `Interzonal Group A-B` intercaladas, así que hay que mirarla antes de forzar el mapa. Lo bueno: la regla del interzonal ya está escrita y la temporada **lee 400 partidos** limpios (144/112/144). Lo que frena el mapa es que RSSSF **le pega la cancha al nombre del club** — `Gimnasia y Esgr. (CdU) (at Huracán-C)`, `(at Libertad)` —, así que el Group B declara 18 nombres donde hay 16. La sede **ya se despega** y `Talleres` resultó ser el `(P)` olvidado en una sola línea. Queda sólo escribirlo, con tres cosas ya sabidas: la correspondencia **no es por letra** —la `Zona A` de la página son los clubes del `Group C` de RSSSF, así que hay que aparear por plantel como en 2008-09—; RSSSF escribe el mismo club de dos formas (`Gimnasia y Esgr. (CdU)` y `Gimnasia y Esgrima (CdU)`), que el mapa resuelve con dos claves; y el `eq=` de la página trae un wikilink con `|` adentro, así que el censo tiene que leer el enlace y no cortar en el primer pipe.
