...
|---|---|---|---|---|
| 43 764 | 107 | 149 | 807 | 285 |

## 2004–2026 — Cerrado

**Todo lo que Wikipedia da de esta era está adentro**

`41 295` partidos. Lo único que queda son verificaciones de calidad, no partidos faltantes: del corpus entero quedan `289` avisos abiertos y cero graves, y todos los que quedan pueden mover un dato.
...
- **Las fases finales que sólo existen como cuadro: quedan 3, y ya no están bloqueadas.** Eran cinco. El lector de tablas de llaves destrabó la del `Argentino A 2012-13`, y ahora **la del `Argentino A 2005-06` entró entera desde RSSSF**: 58 partidos con localía y fecha de día, la temporada pasó de 279 a 337 y los avisos de *el cuadro dice otro marcador* bajaron de 56 a 36. El problema es siempre el mismo — un cuadro dibuja quién jugó contra quién, pero no dice quién fue local, y la convención *arriba es local en la ida* acierta apenas **55.6%** contra 761 patas donde la grilla sí lo dice —.
  - **Las tres que quedan ya tienen fuente, verificada.** `Argentino A 2011-12` está en [rsssf.org/tablesa/arg2012.html#arga](https://www.rsssf.org/tablesa/arg2012.html#arga) —RSSSF cortó la serie `arg3-intNN` después de 2009-10 y la metió como sección de la página del año; `arg3-int2012` y compañía dan 404—. `Primera C 2008-09` está en [arg4-09](https://www.rsssf.org/tablesa/arg4-09.html), **ojo con `arg4-int09`, que es el Argentino B**. Y `Primera C 2011-12` RSSSF no la cubre: la tiene el feed de ESPN (`arg.4`), que el repo ya usa, con la localía en `homeAway` —**no** en `venue`, que en dos partidos nombra una cancha que no es de ninguno de los dos—.
  - **Lo que hay que resolver para cada una.** El mapa de nombres cortos por zona, que es lo que evita meter el club equivocado; y en Primera C 2008-09, cinco partidos jugados en cancha de un tercero (`[at Quilmes]`, `[at Temperley]`…) que necesitan decisión explícita sobre `neutral` en vez de heredar `false`.
  - **Dos trampas ya pagadas, para no volver a pagarlas.** RSSSF no escribe el año en los corchetes (`[May 26]`, no `[May 26, 2012]`), así que hay que deducirlo del arco de la temporada; y sus páginas se sirven en latin-1 sin `meta charset`, así que leerlas como UTF-8 rompe los acentos.
