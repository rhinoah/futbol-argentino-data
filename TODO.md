...
|---|---|---|---|---|
| 44 021 | 36 | 149 | 925 | 371 |

## 2004–2026 — Cerrado

**Todo lo que Wikipedia da de esta era está adentro**

`41 552` partidos. Lo único que queda son verificaciones de calidad, no partidos faltantes: del corpus entero quedan `200` avisos abiertos y cero graves.
...
- **Quedan 36 partidos sin fecha, y 21 son de una sola temporada.** Eran `148`. Casi nada de lo que se cerró necesitó una fuente nueva: estaba en fuentes que el repo ya bajaba y no leía bien. La fase regular de los archivos de RSSSF **no lleva encabezado de zona** cuando el torneo no tiene zonas, y sin esa clave el lector devolvía cero partidos sin un aviso; desde 2010-11 las divisiones **comparten archivo** y hay que acotar la sección —por arriba *y por abajo*—; un título como `Third Phase Reválida` no era título de nada; «repetido» incluía al partido que la página publica **sin día**; y los archivos de 2007-08 escriben la fecha **entre paréntesis** donde los demás usan corchetes, así que las cuatro promociones de la B Nacional 2007-08 se caían con un *«viene sin fecha; queda afuera»*. Lo que queda: 21 del `Argentino A 2004-05` —las patas que la fuente citada no publica—, **6 que no esperan ninguna fecha porque no se jugaron** (los que el Federal A 2024 le dio por perdidos a Sansinena, hoy con `status = no disputado`), 2 del `2010-11` que son un empate con la localía espejada entre las dos fuentes, y 7 sueltos.
