...
|---|---|---|---|---|
| 43 780 | 107 | 149 | 814 | 291 |

## 2004–2026 — Cerrado

**Todo lo que Wikipedia da de esta era está adentro**

`41 311 ` partidos. Lo único que queda son verificaciones de calidad, no partidos faltantes: del corpus entero quedan `291` avisos abiertos y cero graves, y todos los que quedan pueden mover un dato.
...
- **Las fases finales que sólo existen como cuadro: quedan 2.** Eran cinco. El lector de tablas de llaves destrabó la del `Argentino A 2012-13`, y desde RSSSF entraron enteras la del **`Argentino A 2005-06`** (58 partidos) y las semis y la final del **`Argentino A 2011-12`** (6). Los avisos de *el cuadro dice otro marcador* bajaron de **56 a 33**. El problema es siempre el mismo — un cuadro dibuja quién jugó contra quién pero no dice quién fue local, y la convención *arriba es local en la ida* acierta apenas el **55.6%** contra 761 patas donde la grilla sí lo dice.
  - **Las dos que quedan ya tienen fuente verificada.** `Primera C 2008-09` está en [arg4-09](https://www.rsssf.org/tablesa/arg4-09.html) —**ojo con `arg4-int09`, que es el Argentino B**—, y `Primera C 2011-12` RSSSF no la cubre: la tiene el feed de ESPN (`arg.4`), con la localía en `homeAway` y **no** en `venue`, que en dos partidos nombra una cancha que no es de ninguno de los dos. En 2008-09 hay además cinco partidos jugados en cancha de un tercero (`[at Quilmes]`, `[at Temperley]`…) que necesitan decisión explícita sobre `neutral` en vez de heredar `false`.
  - **El mecanismo ya está hecho y probado.** `rsssf.leer_llaves` + `build.sin_repetir`: importa sólo lo que la página no tiene, cruzando por par de clubes **más** fecha y comparando **canonizado** —en crudo el cruce reconocía 8 de 34 y entraban 26 duplicados—. Lo que falta para cada temporada es el mapa de nombres cortos, que es lo que evita meter el club equivocado.
  - **Seis partidos del 2011-12 quedaron con las dos fuentes contradiciéndose**, y no sólo en la fecha: la página dice `Racing (O) 0-2 Central Córdoba (SdE)` el 19/5 y RSSSF `Central Córdoba (SdE) 2-0 Racing (O)` el 18/5 — mismo resultado, cancha al revés. Se conserva el de la página y el aviso muestra las dos versiones. Arbitrarlos pide un tercer testigo.
