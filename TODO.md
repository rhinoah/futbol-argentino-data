...
|---|---|---|---|---|
| 43 790 | 107 | 149 | 817 | 294 |

## 2004–2026 — Cerrado

**Todo lo que Wikipedia da de esta era está adentro**

`41 311 ` partidos. Lo único que queda son verificaciones de calidad, no partidos faltantes: del corpus entero quedan `296` avisos abiertos y cero graves, y todos los que quedan pueden mover un dato.
...
- **Las fases finales que sólo existen como cuadro: queda 1.** Eran cinco. Desde RSSSF entraron enteras la del **`Argentino A 2005-06`** (58 partidos), las semis y la final del **`Argentino A 2011-12`** (6) y el **Reducido de la `Primera C 2008-09`** (10); el lector de tablas de llaves ya había destrabado la del `Argentino A 2012-13`. Los avisos de *el cuadro dice otro marcador* bajaron de **56 a 26**.
  - **La que queda es `Primera C 2011-12`, y RSSSF no la cubre.** La tiene el feed de ESPN (`arg.4`), que el repo ya usa, con la localía en `homeAway` y **no** en `venue` —que en dos partidos nombra una cancha que no es de ninguno de los dos—. Las fechas del feed vienen en UTC: la vuelta de la final es `2012-06-22T00:00Z`, que en hora argentina es el **21**, así que usar la fecha cruda mete un día de más.
  - **Cinco partidos del Reducido 2008-09 se jugaron en cancha de un tercero** (`[at Quilmes]`, `[at Temperley]`…). No se marcan `neutral` —en este repo eso sale del *reglamento* de la competencia y no del estadio, y una mudanza puntual de un partido de liga sigue siendo `false`— ni van a `venue`, porque RSSSF nombra al club anfitrión y no a la cancha. Quedan dichos en un aviso: el hecho no se pierde y no se afirma de más.
  - **Seis partidos del 2011-12 tienen a las dos fuentes contradiciéndose**, y no sólo en la fecha: la página dice `Racing (O) 0-2 Central Córdoba (SdE)` el 19/5 y RSSSF `Central Córdoba (SdE) 2-0 Racing (O)` el 18/5 — mismo resultado, cancha al revés. Se conserva el de la página. Arbitrarlos pide un tercer testigo.
## 1997–2003 — Cerrado
